...
def general():
    ...
    while True:
        ...
        if current_key != None:
            ...
            #print(input_list)
            x.append(xkey)
            if len(input_list) == passlength(password):
                if stringconverter(input_list,passlength(password)) == password:
            ...
            if current_key == ['*','#']:
                set_password.clear()
                scrollmessage('Password reseted, please enter a new password up to 16 characters ')
                lcd.clear()
                time.sleep(1)
                ...
                while True:
                    ...
                    if len(set_password) == 16:
                        input_list.clear()
                        scrollmessage(f'Password set to {stringconverter(set_password,passlength(set_password))} ')
...
def stop():
    ...
    if thisKey != None:
        ...
        if len(disarmCode) > passlength(password):
            lcd.clear()
            printmessage('Incorrect')
            time.sleep(1)
            lcd.clear()
...
def disarm():
    if stringconverter(stop(),passlength(password)) == password:
        #print('Stop')
        servo_write(servo,90)
        unlockChime()
        lcd.clear()
